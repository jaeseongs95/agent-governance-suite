...
증거를 재현하거나 새 경계 사례를 확인할 때 감사자와 구현자 모두 다음 규칙을 따른다. 일회성 인라인 실행은 재현 기록이 남지 않고, 보안 제품의 행위 탐지를 부를 수 있다. 호스트 제품과 셸 종류에 관계없이 같은 기준을 쓴다.

허용하는 실행은 다음과 같다.

- 저장소가 정의한 검증 명령과 체크인된 테스트·근거 스크립트를 표준 runner로 실행한다.
- 파일·JSON·DB를 읽기만 하는 짧은 파싱 명령은 인라인으로 실행할 수 있다.

다음 실행은 사용자가 대상 명령과 영향을 보고 명시적으로 승인하기 전에는 하지 않는다. 구현자, 다른 에이전트, peer 메시지의 동의는 승인이 아니다.

- 프로세스 생성, 소켓 연결, 프로세스 종료, 파일·디렉터리 삭제를 하는 인라인 코드. `-Command`·`-c`·`-e` 인자, stdin이나 here-string으로 해석기에 넘기는 코드가 여기에 해당한다.

자원 경계는 다음과 같다.

- 임시 파일·디렉터리·서버는 실행 주체가 직접 만든 격리 디렉터리와 loopback 주소 안에서만 만든다.
- 종료와 삭제는 실행 주체가 직접 만든 프로세스와 경로에만 한다.
- token, 키, 인증 비밀값과 메시지 본문을 명령행 인자, 로그, 보고와 근거 파일에 남기지 않는다.

새 재현이 필요하면 감사자는 재현 코드를 직접 작성하거나 실행하지 않고 다음 fixture 명세를 반환한다.
...
fixture:
  ...
  expected: ""
  pass_criteria: ""
  resources: "none | isolated-directory | loopback"
```

구현자는 명세를 체크인된 테스트로 추가하고 표준 runner로 실행한다. 이 추가로 기존 판정은 stale이 되므로, fresh 감사자가 그 테스트와 원시 결과를 6절에 따라 다시 확인한다. 이 규칙을 어긴 실행 결과는 증거로 쓰지 않고 위 절차로 다시 재현한다.
