...
3. 호스트 runtime metadata, 사용자 텍스트나 이번 요청의 화면 캡처에서 현재 task의 모델과 추론 수준을 모두 관측한 경우 `model-effort-fit-assessment`를 요청한다. 현재 선택이 없으면 일반 direct 작업에서는 이 capability 때문에 묻거나 작업을 멈추지 않으며, 결과가 `ADEQUATE`이면 사용자 안내를 생략한다. 단, MCP `orchestrated` workflow를 계획할 때는 별도 규칙을 적용한다. 현재 bootstrap 판단을 수행한 모델·model class·추론 수준을 호스트가 관측한 `ExecutionContext.v1`으로 결속해 `plan_workflow` wrapper의 `executionContext`에 전달한다. 관측값을 만들 수 없거나 MCP가 반환한 최소 semantic assurance 하한보다 낮으면 임의로 보정하거나 추정하지 않고 해당 workflow를 시작하지 않는다.
...
- MCP 계획 stage에 `executionRequirement`가 있으면 그 stage를 실제 수행한 실행자의 모델·model class·추론 수준을 host runtime 또는 spawn 결과에서 직접 관측해 `StageResult.v1.executionContext`로 제출한다. 현재 세션 값, 다른 worker 값이나 모델 이름에서 추측한 class를 대신 쓰지 않는다. semantic stage의 관측값이 없거나 하한 미달이면 `passed` 결과를 제출하지 않는다. `korean-prose-finalization`처럼 계획이 `deterministic`으로 표시한 stage에는 이 관측값을 요구하지 않는다.
