...
이 스킬의 selection, editing, verification, finalization provider는 활성 상태다. 직접 호출과 암시 호출 모두 아래 계약을 지키며 실행한다. 품질 평가에서는 활성화된 동일 후보를 동결하고, 사전에 정한 기준을 충족하지 못하면 그 결과를 실패로 기록한다.
