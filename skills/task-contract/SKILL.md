...
4. [risk-and-authorization-rubric.md](references/risk-and-authorization-rubric.md)에 따라 위험과 권한을 분리한다. 각 action에는 요청 입력의 `authorizationEvidence`와 정확히 일치하는 `authorizationProvenance`를 하나만 연결한다. 프로젝트 지침은 행동을 제한할 수 있지만 `allowedActions`를 늘리는 권한 근거로 쓰지 않는다. 출처 receipt가 있으면 `sourceReceiptId`를 보존하되, receipt 자체를 사용자 승인으로 승격하지 않는다.
