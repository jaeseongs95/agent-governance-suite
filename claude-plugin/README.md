...
Claude Code adapter는 args 형식의 SessionStart 훅에서 relay를 띄우고 그 훅의 parent PID와 시작 식별자를 host 생명주기로 사용합니다. 시작 식별자를 얻지 못하면 선택 기능인 relay를 띄우지 않습니다. `CLAUDE_CODE_MESSAGING_SOCKET`과 `CLAUDE_CODE_MESSAGING_TOKEN`은 Claude host, 짧게 실행되는 hook과 relay 환경에만 남기고 broker 자식 환경에서는 제거합니다. inbox에는 본문이 아니라 broker가 발급한 nonce의 wake bell만 보내며, 실제 본문은 공용 TLS 1.3 broker에서 동기 훅이 한 건씩 claim합니다. 최대 envelope를 9,000 UTF-16 code unit 이하로 고정해 10,000자 `additionalContext` 한도에 여유를 둡니다. TLS spool과 ACK가 본문 전달의 내구성을 맡고 wake는 유실 가능한 힌트입니다. broker는 target마다 소비되지 않은 wake를 하나만 예약하고, 실제 UserPromptSubmit 훅이 nonce를 소비한 뒤에만 다음 wake를 허용합니다. 전송 시작 전의 확정 실패만 예약을 풀고 지수 backoff로 재시도하며, 전송 결과가 불명확하면 중복을 피하려고 다시 보내지 않습니다. 합쳐진 여러 bell은 줄 단위로 검증합니다. 모델이 처리한 `messageId`를 `acknowledge_session_messages`로 ACK하기 전까지 TTL 안에서 재전달될 수 있습니다. inbox write는 ACK로 간주하지 않습니다.
...
- 이 경로의 추론 수준은 훅 입력의 `effort.level`만 쓴다. 이전 메시지의 effort는 다른 모델의 값일 수 있어서다. 이전 메시지는 `sessionId`가 일치하고 현재보다 과거 시각이 적힌 것만 쓴다. 호출을 낸 메시지가 transcript에 있는데 다른 세션·sidechain이거나 모델이 없어 거부되면 이 경로로 넘어가지 않는다.
- 한계: 전환 기록이 빠지면(훅 실패·시간 초과, 기록 쓰기 실패) 이전 모델로 증명된다. 실측한 것은 `SessionStart`(startup)와 `/model` 전환뿐이고, resume·clear·fork의 `SessionStart`, `--resume --model`, 자동 fallback 전환은 확인하지 않았다. 또 호출 전에는 알 수 없는 `tool_use_id`가 필요 없으므로, 같은 사용자 권한으로 transcript에 줄을 덧붙이거나 기록 파일을 쓰면 이전보다 쉽게 모델을 위조할 수 있다. 아래 위협 모델의 범위 안이다.
