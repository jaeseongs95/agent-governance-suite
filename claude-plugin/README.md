...
- 토큰을 만들지 못하면 훅은 호출자가 도구 인자에 넣은 `_hostAttestation`을 지우고 넘긴다. launcher도 훅이 실패하거나 시간을 넘기거나 `CLAUDE_PLUGIN_DATA`가 없을 때 같은 처리를 한다. 따라서 모델이 도구 인자로 토큰을 넣어도 서버에 닿지 않는다.
