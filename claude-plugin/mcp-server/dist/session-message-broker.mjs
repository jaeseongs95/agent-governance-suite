...
var SessionMessageStore = class {
  ...
  acquireRelay(input, nowMs = Date.now()) {
    ...
    const result = this.database.prepare(`INSERT INTO relay_leases
      ...
      WHERE relay_leases.lease_until <= excluded.updated_at OR relay_leases.relay_id = excluded.relay_id`).run(input.host, input.sessionId, input.transport, input.relayId, input.pid, input.parentPid, until, now);
  ...
  consumeWake(target, nonce, nowMs = Date.now()) {
    ...
    const result = this.database.prepare(`UPDATE wake_nonces SET consumed_at = ?
      WHERE nonce_digest = ? AND host = ? AND session_id = ? AND consumed_at IS NULL AND expires_at > ?`).run(iso(nowMs), nonceDigest(nonce), target.host, target.sessionId, iso(nowMs));
    return result.changes === 1;
  }
};
...
function dispatch(store, operation, payload) {
  switch (operation) {
    case "ping":
    ...
    case "claim":
      return { messages: store.claim(identity(payload.target)) };
    case "acknowledge":
...
`);
  ...
  const temporary = `${endpointPath}.${process.pid}.${createHash2("sha256").update(String(Date.now())).digest("hex").slice(0, 8)}.tmp`;
