...
3. [resolution-model.md](resolution-model.md)에 따라 넓은 범위의 규칙부터 적용한다.
4. 자연어 규칙이 충돌하면 [conflict-classification.md](conflict-classification.md)에 따라 `activeRules`, `overriddenRules`, `unresolvedConflicts`를 채우고 `analysisStatus`를 `complete`로 바꾼다. 스크립트 결과만으로 의미 충돌을 해소했다고 주장하지 않는다.
