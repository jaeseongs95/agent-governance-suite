...
description: 스킬·모델 평가 결과를 품질 근거나 릴리스 근거로 제출하려 할 때, 평가를 설계해 실행하기 전, 평가가 공정했는지 의심될 때 쓴다. 입력·판정 방법·집계가 공정하고 재현 가능한지 실행 전후에 감사한다. 평가 실행·채점·수정과 일반 구현 검증은 하지 않는다.
license: MIT
metadata:
  version: "1.0.0"
---
