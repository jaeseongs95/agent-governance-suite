...
- 적용 가능한 상위 지침, `CLAUDE.md`(import된 `AGENTS.md` 포함), 사용자 권한과 승인 범위를 먼저 따른다.
- 감사자는 구현·수정·구현자 검증에 참여한 에이전트와 다른 실제 worker여야 한다. 역할 이름만 바꾼 자기감사는 독립 감사가 아니다.
- 감사자는 fresh context로 시작해야 한다. Agent 도구로 fork가 아닌 새 서브에이전트를 시작한다. 가능하면 `agent-governance-suite:independent-auditor`를 사용하고, 재위임과 하위 에이전트 생성을 금지한다. 부모 대화를 상속하는 fork(`#fork`, `/subtask`)는 감사자로 쓰지 않는다.
