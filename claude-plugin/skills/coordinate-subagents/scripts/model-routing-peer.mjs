import { assert, keys, text, validateBinding, validateTarget, digestValue, canonical, verifySeal, revalidateDispatch } from './model-routing-core.mjs';
...
export async function acceptPeerAssignment(body,{features,target,loadAssignment,environment,authorizeAndConsume,store}){
  ...
  let allowed=false;
  try{allowed=await authorizeAndConsume({request,decision,target});}catch{allowed=false;}
  ...
  const current=typeof environment.refresh==='function'?await environment.refresh():environment;
  try{revalidateDispatch(request,decision,current);}catch(error){store.transition(reservation.dispatchKey,0,'not-started','existing-workflow:revalidation-failed');throw error;}
