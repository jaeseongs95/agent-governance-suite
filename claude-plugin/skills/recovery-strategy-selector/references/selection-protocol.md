...
각 action은 요청의 authorization evidence 하나와 정확히 대응해야 한다. 전략의 `addressesCauseId`는 확정 diagnosis의 `causeId`와 같아야 하고, `rootConditionDigest`는 확정 `{ causeId, rootCondition }`의 canonical digest와 같아야 하며, `rootConditionChange`는 그 근본 조건을 바꾸는 작동 방식을 설명해야 한다. 프로젝트 지침은 행동을 제한할 수 있지만 허용 권한을 새로 만들 수 없다. 승인으로 해소 가능한 행동은 전략에 남길 수 있으나 handoff만으로 승인됐다고 보지 않는다.
