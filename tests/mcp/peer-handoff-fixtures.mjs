...
export function createPeerWorkflow(directory,{now=Date.now(),target=RECEIVER,highRisk=false,authorization,catalog=PEER_CATALOG}={}){
  ...
  const env=environment({catalog:structuredClone(catalog),capabilities:[cap],now:new Date(now).toISOString()}),decision=resolveV2(req,env);
