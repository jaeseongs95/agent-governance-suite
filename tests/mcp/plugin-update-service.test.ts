...
describe("PluginUpdateService", () => {
  ...
  it("preserves the last successful result and retries one hour after failure", async () => {
    ...
    const service = new PluginUpdateService(store, {
      ...
      fetcher: async () => {
        ...
        return jsonResponse([reference("v1.4.0")]);
      },
    });
  ...
  it("emits each available version once and persists that claim", async () => {
    ...
    expect(notice).toMatchObject({ latestVersion: "1.4.0", automaticInstall: false });
    ...
    expect(restarted.takeNotice(next)).toMatchObject({ latestVersion: "1.5.0" });
  });
  ...
  it("preserves a concurrent SQLite notice claim against a stale check write", async () => {
    ...
    firstResponse.resolve(jsonResponse([reference("v1.4.0")]));
    const firstStatus = await firstCheck;
    expect(firstService.takeNotice(firstStatus)?.latestVersion).toBe("1.4.0");

    secondResponse.resolve(jsonResponse([reference("v1.4.0")]));
    const secondStatus = await secondCheck;
    expect(secondService.takeNotice(secondStatus)).toBeNull();
    expect(secondStore.getPluginUpdateState("agent-governance-suite")?.lastNotifiedVersion).toBe("1.4.0");
    firstStore.close();
    secondStore.close();
  });
  ...
  it("preserves a concurrent successful SQLite result when a stale check fails", async () => {
    ...
    successResponse.resolve(jsonResponse([reference("v1.4.0")]));
    expect(await successfulCheck).toMatchObject({ latestVersion: "1.4.0", lastErrorCode: null });

    failureResponse.reject(new TypeError("offline"));
    expect(await failedCheck).toMatchObject({ latestVersion: "1.4.0", lastErrorCode: "NETWORK" });
  ...
  it("does not notify an older version after a higher version was already claimed", async () => {
    ...
    expect(versions).toEqual(["1.4.0", "1.5.0", undefined]);
    expect(store.getPluginUpdateState("agent-governance-suite")?.lastNotifiedVersion).toBe("1.5.0");
  });
  ...
  it("uses volatile state when update storage fails", async () => {
    ...
    const service = new PluginUpdateService(failingStore, {
      fetcher: async () => jsonResponse([reference("v1.4.0")]),
    });
    const status = await service.check();
  ...
  it("reuses a cached ETag after a 304 response", async () => {
    ...
    expect(status).toMatchObject({ latestVersion: "1.4.0", stale: false, lastErrorCode: null });
  });
  ...
  it("dereferences an annotated stable tag to its commit", async () => {
    ...
    expect(await service.check()).toMatchObject({ latestVersion: "1.4.0", latestCommit: commitSha });
  });
});
