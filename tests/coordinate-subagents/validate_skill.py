...
def read_utf8(path: Path) -> str:
    ...
    try:
        ...
        raise ValidationError(
            f"file is not valid UTF-8: {path.relative_to(MONOREPO_ROOT)}"
        ) from error
...
def validate_frontmatter() -> None:
    ...
    require(isinstance(version, str) and re.fullmatch(r"0\.1\.1", version) is not None,
            "SKILL.md frontmatter version (version or metadata.version) must be '0.1.1'")
    ...
    for markdown in sorted(SKILL_ROOT.rglob("*.md")):
        ...
        for raw_target in MARKDOWN_LINK.findall(read_utf8(markdown)):
            ...
            if path_part and not (markdown.parent / path_part).resolve().exists():
                failures.append(
                    f"{markdown.relative_to(MONOREPO_ROOT)} -> {target}"
                )
    ...
    for path in skill_text_files():
        ...
        for line_number, line in enumerate(read_utf8(path).splitlines(), start=1):
            if marker.search(line):
                failures.append(f"{path.relative_to(MONOREPO_ROOT)}:{line_number}")
    ...
    for path in skill_text_files():
        ...
        for label, pattern in {**signatures, **personal_paths}.items():
            if pattern.search(text):
                failures.append(f"{path.relative_to(MONOREPO_ROOT)}: possible {label}")
        ...
        for line_number, line in enumerate(read_utf8(path).splitlines(), start=1):
            ...
            for marker in POLICY_MARKER.findall(line):
                require(marker not in discovered,
                        ...
                        f"{path.relative_to(MONOREPO_ROOT)}:{line_number}")
                discovered[marker] = {
                    "source": path.relative_to(MONOREPO_ROOT).as_posix(),
...
def read_markdown_section(path: Path, section: str) -> str:
    ...
    for line in read_utf8(path).splitlines():
        ...
        if active:
            lines.append(line)
    require(active or lines, f"missing Markdown section {section!r} in {path.relative_to(MONOREPO_ROOT)}")
    return " ".join("\n".join(lines).split())
...
def validate_behavior_cases() -> None:
    path = TEST_ROOT / "behavior-cases.yaml"
    try:
        document = json.loads(
            read_utf8(path),
        ...
        raise ValidationError(
            "tests/coordinate-subagents/behavior-cases.yaml must remain valid JSON (and therefore valid YAML 1.2)"
        ) from error
    ...
    for contract_id, declaration in declared_contracts.items():
        ...
        source = MONOREPO_ROOT / str(declaration["source"])
