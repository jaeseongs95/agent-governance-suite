...
const request = {
  ...
  sender, recipient, callbackTarget: sender, revision: 1,
...
const outcome = {
  ...
  taskId: request.taskId, actor, callbackTarget: sender, revision: 2,
...
const requestContext = { authenticatedActor: sender, currentInstanceId: sender.instanceId, currentRevision: 0 };
const outcomeContext = { authenticatedActor: actor, currentInstanceId: actor.instanceId, currentRevision: 1, currentTaskId: request.taskId, request };
...
test('W01 host-neutral request, terminal report and activity schemas reject implicit completion and host extensions', () => {
  ...
  assert.equal(validate({ ...request, taskId: 'W01', sender: { ...sender, instanceId: 'i' }, callbackTarget: { ...sender, instanceId: 'i' } }), true);
...
test('W01 callback is bound to authenticated sender and stored request', () => {
  ...
  assert.throws(() => assertSessionTaskTransitionV1({ ...request, callbackTarget: actor }, requestContext), /CALLBACK_TARGET_MISMATCH/);
  assert.throws(() => assertSessionTaskTransitionV1({ ...outcome, callbackTarget: actor }, outcomeContext), /REQUEST_BINDING_MISMATCH/);
...
test('W01 standalone task can report outcome without inventing a callback request', () => {
  ...
  assert.throws(() => assertSessionTaskTransitionV1(standalone, { ...outcomeContext, request: undefined, currentTaskId: 'other' }), /REQUEST_BINDING_MISMATCH/);
});
...
test('W01 old instance and revision cannot replace current activity or outcome', () => {
  ...
  const activityContext = { authenticatedActor: actor, currentInstanceId: actor.instanceId, currentRevision: 0, observedSource: 'host-observed' };
