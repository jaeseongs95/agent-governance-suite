...
현재 공개 릴리스는 `v1.15.1`이며 거버넌스 전문 스킬 15개, 로컬 task continuity 인프라 스킬 1개와 한국어 산문 워크플로 1개를 포함합니다. 이번 릴리스는 Claude Code 배포물에 스킬 유도 훅을 추가합니다. 요청 문장과 실행할 `Bash` 명령에서 커밋·삭제·배포·반복 실패·미정 구현 요청 패턴을 찾아 적용 가능한 스킬 이름을 컨텍스트로 한 번 안내하며, 차단이나 상태 기록은 하지 않습니다. 스킬 구성, 계약 schema, 용어집 데이터와 SQLite schema는 v1.15.0과 같고 Codex 배포물과 MCP 서버 동작은 바뀌지 않습니다.
<!-- release-version:end -->
