...
현재 공개 릴리스는 `v1.1.0`입니다. 거버넌스 전문 스킬 10개와 한국어 산문 워크플로 1개를 포함합니다. 한국어 산문 워크플로는 품질 평가의 개선율 기준을 충족할 때까지 `skills/registry.json`에서 비활성화되어 MCP 라우팅과 오케스트레이터의 선택 대상에서 제외됩니다.
