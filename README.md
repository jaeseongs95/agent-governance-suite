...
현재 공개 릴리스는 `v1.5.0`입니다. 거버넌스 전문 스킬 12개, 로컬 task continuity 인프라 스킬 1개와 한국어 산문 워크플로 1개를 포함합니다. 한국어 산문 워크플로는 확장한 SQLite 용어집과 함께 활성 후보로 평가했지만 편집 계약 검증을 통과하지 못했으므로, registry, 직접 descriptor와 Codex 암시 호출 설정에서 다시 비활성화했습니다. 이 변경은 아직 원격 릴리스로 배포되지 않았습니다.
<!-- release-version:end -->
