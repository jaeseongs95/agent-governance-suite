...
현재 공개 릴리스는 `v1.5.0`입니다. 거버넌스 전문 스킬 12개, 로컬 task continuity 인프라 스킬 1개와 한국어 산문 워크플로 1개를 포함합니다. 현재 `main` 후보에서는 한국어 산문 워크플로의 registry, 직접 descriptor와 Codex 암시 호출을 활성화했으며, 신규 holdout 품질 평가는 이 활성 후보를 그대로 동결해 수행합니다. 이 변경은 아직 원격 릴리스로 배포되지 않았습니다.
<!-- release-version:end -->
