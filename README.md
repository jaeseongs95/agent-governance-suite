...
현재 공개 릴리스는 `v1.9.0`입니다. 거버넌스 전문 스킬 12개, 로컬 task continuity 인프라 스킬 1개와 한국어 산문 워크플로 1개를 포함합니다. 한국어 산문 워크플로는 품질 평가의 개선율 기준을 충족할 때까지 registry, 직접 descriptor와 Codex의 암시적 호출 설정에서 비활성화됩니다. 사용자가 직접 호출해도 스킬의 차단 지침에 따라 provider와 로컬 스크립트를 실행하지 않습니다.
<!-- release-version:end -->
