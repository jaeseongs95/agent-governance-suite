...
전문 스킬을 직접 사용하려면 `$coordinate-subagents`, `$independent-deliberation-panel`, `$independent-audit-gate` 중 필요한 스킬을 지정합니다. 둘 이상의 역할을 연결하고 실행 순서와 증거를 관리하려면 `$orchestrator`를 사용합니다.
...
설치 전에 `node --version`으로 Node.js 22 이상인지 확인합니다. 설치 후 저장소나 설치 캐시의 플러그인 루트에서 `node scripts/check-runtime.mjs`를 실행하면 Node.js 버전, MCP 설정, 번들과 registry 존재 여부를 검사합니다. Node.js가 없으면 [Node.js 다운로드 페이지](https://nodejs.org/en/download)에서 LTS 버전을 설치한 뒤 새 터미널에서 다시 확인합니다. MCP가 시작되지 않아도 세 전문 스킬의 직접 호출은 사용할 수 있습니다.
...
별도 저장소에서 개발한 스킬은 깨끗한 태그나 커밋만 편입합니다. 처음 편입하는 capability에는 `--phase`와 `--capability`를 함께 지정합니다. 이미 편입한 스킬을 새 ref로 교체할 때는 `--replace true`를 명시합니다.
...
편입 명령은 작업 트리를 복사하지 않고 지정한 Git ref를 archive로 읽습니다. `.git`, `__pycache__`, `evals/results`, 일반 빌드 결과는 제외하고, 원본 ref·commit·checksum을 `skills/source-lock.json`에 기록합니다. 편입 PR에서는 자동 생성된 기본 fixture를 실제 스킬의 동작 사례로 교체해야 합니다.
...
공개 계약은 `contracts/`에 JSON Schema 2020-12로 정의되어 있습니다. 상태와 오류 코드는 계약에 선언된 값만 사용하며, 호환 가능한 스킬 추가는 minor, 수정은 patch, 계약·권한·식별자의 호환되지 않는 변경은 major 버전으로 관리합니다.

글로벌 작업 지침에서 분리한 후속 일곱 스킬은 [추가 스킬 구현 계획](docs/additional-skills-implementation-plan.md)에 정리했습니다. 각 스킬을 독립 저장소에서 먼저 완성한 뒤 integration descriptor와 clean tag로 이 플러그인에 편입하는 순서를 따릅니다.
