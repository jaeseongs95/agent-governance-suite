...
The current public release is `v1.17.0` and includes fifteen governance specialist skills, one local task-continuity infrastructure skill, and one Korean prose workflow. This release adds a host attestation hook to the Claude Code distribution, so orchestrated workflows that require execution assurance start with the model and reasoning effort observed by the harness. The Codex distribution behaves as in v1.16.2. The candidate-v2 policies applied to `korean-prose-editor` in v1.16.0 were not covered by the quality-gate pass (`0.3.0-gate-1`) and have not been quality-evaluated yet.
<!-- release-version:end -->
...
- Orchestrated workflows that require execution assurance start with the model and reasoning effort observed by a Claude-only host attestation hook. Claude Code runs the hook right before `plan_workflow` and `record_stage_result`; it reads the model of the assistant message that issued the call from the transcript and the effort from the hook input (or the transcript entry), and injects a signed token into the tool input. The server verifies the token only when the Claude distribution passes `AGENT_GOVERNANCE_HOST_ATTESTATION=claude-code`, so the Codex distribution server still returns `BINDING_REQUIRED`. The token shows harness observation only; its signing key lives in a state database readable by the same OS user, so it is not OS-level tamper protection.
