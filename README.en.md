...
- Orchestrated workflows that require execution assurance start with the model and reasoning effort observed by a Claude-only host attestation hook. Claude Code runs the hook right before `plan_workflow` and `record_stage_result`; it reads the model of the assistant message that issued the call from the transcript and the effort from the hook input, and injects a signed token into the tool input. The server verifies the token only when the Claude distribution passes `AGENT_GOVERNANCE_HOST_ATTESTATION=claude-code`, so the Codex distribution server still returns `BINDING_REQUIRED`. The token shows harness observation only; its signing key lives in a state database readable by the same OS user, so it is not OS-level tamper protection.
