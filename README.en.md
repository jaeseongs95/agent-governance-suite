...
- With the same value the server also advertises session `instructions`, which Claude Code places in its system prompt at session start: an intake rule to classify the failure impact of a request before editing files or running commands, and to call the orchestrator or the matching specialist first when the impact is high. Without the value the server advertises no `instructions`.
