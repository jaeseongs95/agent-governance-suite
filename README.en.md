...
The current public release is `v1.18.0` and includes fifteen governance specialist skills, one local task-continuity infrastructure skill, and one Korean prose workflow. This release makes MCP orchestrated workflows work end to end in Claude Code: the Claude orchestrator guidance now uses that path, tool schemas are advertised without the `$ref` that Claude Code cannot resolve, and host attestation handles effort, model IDs and validity windows more completely. `mutation-risk-preflight` moves to upstream v1.0.1. Apart from that skill fix, the Codex distribution behaves as in v1.17.0. The candidate-v2 policies applied to `korean-prose-editor` in v1.16.0 were not covered by the quality-gate pass (`0.3.0-gate-1`) and have not been quality-evaluated yet.
<!-- release-version:end -->
