...
The current public release is `v1.4.1`. It includes eleven governance specialist skills, one local task-continuity infrastructure skill, and one Korean prose workflow. The Korean prose workflow remains disabled in the registry, its direct descriptor, and Codex's implicit-invocation setting until it passes the prose-quality improvement threshold. Its fail-closed skill instructions also refuse direct calls without running providers or local scripts.
...
```bash
codex plugin marketplace add jaeseongs95/agent-governance-suite --ref v1.4.1
