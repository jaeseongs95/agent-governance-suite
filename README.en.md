...
The current public release is `v1.14.0` and includes fifteen governance specialist skills, one local task-continuity infrastructure skill, and one Korean prose workflow. This release adds a Claude Code distribution: `claude-plugin/` is generated by `pnpm claude:build`, Claude-only files and wording overrides live in `claude-overlay/`, and its state databases are created only under `${CLAUDE_PLUGIN_DATA}`. The Claude distribution alone advertises `plan_workflow` without a top-level schema combinator, so the Anthropic API no longer skips that tool. Skill composition, contract schemas and SQLite schemas are unchanged from v1.13.0, and the Codex distribution behaves exactly as before.
<!-- release-version:end -->
