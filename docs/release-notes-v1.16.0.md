...
`0.3.0-gate-1` 품질 기준 통과는 candidate-v2 이전 정책과 평가 toolchain을 대상으로 한 결과입니다. 이번 릴리스의 정책과 평가 도구는 그 평가 대상과 다르므로, v1.16.0의 `korean-prose-editor`는 품질 미평가 상태입니다. 스킬은 v1.15.x와 같이 활성 상태로 배포하며, 이는 사용자의 명시적 릴리스 요청에 따른 결정입니다. 새 정책은 신규 frame과 private holdout으로 다시 평가해야 합니다.
