...
- host attestation 보완:
  - 추론 수준은 훅 입력과 transcript 메시지의 값 중 낮은 쪽을 씁니다.
...
- 같은 세션에서 서브에이전트가 부른 `plan_workflow`는 서브에이전트 actor로 관측됐습니다. `effort: low`로 정의한 서브에이전트는 훅 입력에 `low`가 들어왔고 하한 미달로 `BINDING_INVALID`였습니다.
- 수정 전 배포본으로는 같은 흐름이 `open_convergence_root`에서 `INVALID_INPUT`("must be object")으로 멈췄습니다.
