...
- 한 줄 갱신은 필수입니다. 사용자 요청마다 처음 파일을 고치거나(Edit·Write·MultiEdit·NotebookEdit) 명령(Bash·PowerShell)이나 서브에이전트(Agent)를 실행하려 할 때 그 요청 뒤 요약이 없으면 훅이 한 번 거부하고 이유를 알립니다. 다음 시도는 허용하므로 현황판에 장애가 있어도 작업은 멈추지 않습니다. 읽기 전용 단일 명령(`git status`·`git log`·`git diff`·`git show`·`git branch`, `ls`, `cat`, `pwd`, `rg`, `grep`)은 거부하지 않습니다. 연산자, 리다이렉션, 변수, 괄호·중괄호, 따옴표·백슬래시가 들어가거나 파일을 쓰거나 프로그램을 실행하는 옵션(`--output`, `--ext-diff`, `--pre`)이 있으면 읽기 전용으로 보지 않습니다. 서브에이전트 호출은 막지 않지만 서브에이전트가 부모 세션의 줄을 바꾸지는 못합니다. 훅의 모든 오류는 호출을 통과시킵니다. 강제 시점과 강도는 독립 숙고 패널(blind reviewer 3명과 fresh Judge)이 정했습니다.
...
- Codex에서는 실행해 확인하지 않았습니다. 훅 번들을 직접 실행해 Codex 형식 입력으로 모의 시험만 했습니다.
  ...
  - Codex가 PreToolUse의 입력 수정(`updatedInput`)을 지원하는지도 확인하지 않았습니다. 지원하지 않으면 행은 훅이 기록하지만 `update_session_status`는 결속 정보를 받지 못해 `BINDING_REQUIRED`를 돌려줍니다.
  - 현황판 도구 결속 훅은 Codex의 MCP 도구 이름을 `mcp__agent-governance-suite__…` 형식으로 가정합니다. 형식이 다르면 결속이 일어나지 않습니다.
...
- 구현 후보에서 `bundle:check`, `claude:drift`, `lint`, `build`, `test`(370건 통과, 새 현황판 테스트 11건과 경로 테스트 1건 포함), `runtime:check`, `validate:all`, `validate:official`, `claude:check`가 모두 종료 코드 0이었습니다.
- 새 테스트가 확인하는 것:
  - 요청마다 한 번만 거부하고 갱신 뒤 허용한다.
  - 요청 이벤트가 없는 호스트는 세션 시작을 기준으로 삼는다.
  - 한 줄·200자 제한을 지킨다.
  - 목록의 `stale`·`current` 표시와 24시간 정리가 맞다.
  - 두 연결이나 동시에 뜬 훅 프로세스 4개가 같은 요청을 경합해도 거부는 한 번이다.
  - Codex 훅 설정의 모든 명령(`command`, `commandWindows`)이 실제 번들 파일을 가리킨다.
  - 읽기 전용 명령을 판정한다.
  - 서브에이전트는 면제되고 부모의 줄을 쓰지 못하며, 오류가 나면 통과한다.
  - MCP 도구 2개의 기록 확인, `BINDING_REQUIRED`, 설정 없음 처리가 맞고, 목록 도구는 DB 파일을 새로 만들지 않는다.
...
- 릴리스 commit `c573766`에서도 같은 전체 검증과 `release:check`, `source:check`, `source:verify`가 모두 종료 코드 0이었습니다(테스트 370건). push 전 fresh 독립 사전 감사는 세 차례(dc2de74 FAIL, de15df6 PASS, c573766 PASS) 진행했습니다.
