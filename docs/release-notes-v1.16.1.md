...
접수 규칙은 2026년 9월 18일 새 세션 측정에서 정했습니다. CI 워크플로를 고치고 커밋하는 같은 과제를 조건 다섯 개로 각 2회 실행했을 때, 첫 파일 편집 전에 거버넌스 스킬을 스스로 호출한 비율은 현재 플러그인 0/2, 설명문을 Codex 원문으로 되돌린 플러그인 0/2, 프롬프트마다 orchestrator 지침 전문을 넣어 준 플러그인 0/2였고, 프롬프트마다 접수 규칙을 넣은 훅과 서버 `instructions`의 접수 규칙은 각각 2/2였습니다. 열 세션 모두 독립 감사까지는 이어지지 않았고, MCP 도구를 허용해도 같았습니다(2/2). orchestrator를 부른 세션도 그 문서를 읽기만 하고 필요한 단계를 정하지 않은 채 구현으로 갔기 때문입니다. 그래서 과제를 "커밋 후 `main` 병합과 `origin` push"까지로 올리고(복제본마다 로컬 bare 저장소를 `origin`으로 사용) 다시 쟀습니다. 접수 규칙만 있는 사본은 2/2에서 orchestrator를 부르고 push 직전 preflight는 했지만 독립 감사는 0/2였고, 접수 규칙이 orchestrator를 거치게 하고 orchestrator에 선택 결정 절을 넣은 사본은 3/3에서 fresh 감사자 서브에이전트의 독립 감사를 거친 뒤 push했습니다(턴 수는 48~56에서 61~124로 늘었습니다). 사례당 2~3회의 비통계적 관측이며, 원자료는 저장소 밖 `eval-tools/`에 있습니다.
