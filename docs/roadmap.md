...
문서 기준일은 2026년 9월 14일이다. 현재 공개 릴리스는 `v1.10.0`이다. 한국어 산문 워크플로는 확장한 용어집 검증과 사용자의 명시적 배포 승인에 따라 활성화했지만, 신규 holdout의 최종 `≥80%` 지표는 생성되지 않았으므로 품질 기준 통과로 기록하지 않는다.
<!-- release-version:end -->
