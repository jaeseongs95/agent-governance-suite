...
문서 기준일은 2026년 9월 20일이다. 현재 공개 릴리스는 `v2.3.0`이다. v2.2.1은 v2.2.0의 동작을 유지하면서 Claude 생성물의 호스트 경계와 한·영 README 스킬 표를 바로잡고, 인증서 serial DER 정규화로 간헐적인 broker 시작 실패를 제거하는 patch 릴리스다. v2.2 계열은 사용자 홈의 공용 상태 경로, TLS 1.3 loopback broker, 호스트 중립 메시지 계약과 중복 wake 억제를 포함한다. 한국어 산문 워크플로의 candidate-v2 정책은 2026년 9월 17일 동결 frame `0.3.0-gate-1`의 평가 대상이 아니므로 품질 미평가 상태로 유지한다.
<!-- release-version:end -->
