...
| 3.0 이후. 세션 작업 완료와 상태 기반 연락 | 계획 | 작업 종료 신호를 기록하고 수신 세션 상태에 맞게 메시지 전달·깨우기 선택 | MCP 작업 종료 기록, 상태 조회, 기존 broker를 이용한 전달·wake 정책 |
...
- `0.2.0-private-20260914-1`은 신규 30-case private holdout, 독립 corpus 감사와 3회 실행을 완료했지만 terminal `failed`로 종결했다. 보호 문자열은 390/390, legacy restraint는 82/84, holdout restraint는 59/60으로 통과했으나 legacy 의미 보존 295/300, legacy 개선 146/216, holdout 개선 20/30으로 기준에 못 미쳤다. 호스트가 실제 모델과 provider 버전을 증명하지 않아 이 실행은 실제 모델 provenance 근거로도 사용할 수 없다. 같은 frame과 holdout은 재사용하지 않는다.
...
- SQLite 용어집은 공식 프로젝트·표준·TTA·공식 한국어 문서에 근거한 242개 활성 entry(전체 243개, 248개 form)로 확장했다. `protect` 162개, `prefer` 4개, `avoid` 8개(활성), `allow` 68개를 각각 시험하며, 데이터 버전은 `1.2.1`이다.
- 2026년 9월 18일 Claude Code에서 플러그인 스킬이 거의 호출되지 않는 원인을 조사했다. Claude Code는 Codex의 `agents/openai.yaml` 암시 호출 정책(15/18 스킬 `allow_implicit_invocation: true`)을 읽지 않고 `SKILL.md`의 `description` 한 줄로만 스킬을 고르는데, 17개 중 16개 설명문이 제외 조항으로 끝나고 트리거 상황 예시가 없었으며, orchestrated MCP 경로는 `BINDING_REQUIRED`로 막혀 있고 스킬 사용을 유도하는 훅도 없었다. 대응 1(생성 사본의 description을 트리거 상황 우선으로 치환, v1.15.0)을 새 세션(`claude -p --plugin-dir`, 세션마다 fresh, 사례당 1~2회)으로 측정한 결과 설명문 낱말을 그대로 담은 요청 4개에서는 1.14.1 3/4 → 1.15.0 4/4였지만, 짧은 자연어 요청 4개("이제 커밋할게", "바로 rm -rf 해줘", "테스트가 계속 같은 데서 죽어", "일단 알아서")에서는 0/4 → 1/4에 그쳤다. 대응 2로 Claude 전용 `hooks/skill-trigger-hook.mjs`를 추가해 `UserPromptSubmit`과 `PreToolUse`(`Bash`)에서 커밋·삭제·배포·반복 실패·미정 구현 요청 패턴에 해당 스킬 이름을 `additionalContext`로 한 번 안내하게 했고, 같은 어려운 요청 4개에서 2회 모두 3/4가 됐다(스킬이 필요 없는 대조군 요청은 측정한 3회 모두 스킬을 호출하지 않았다). 비교 arm은 사용자 설정을 제외한 세션이어서 기본 모델로 실행됐고, 사례당 1~2회의 비통계적 관측이다. 남은 miss는 "일단 알아서"형 미정 구현 요청으로, 훅 안내가 있어도 모델이 바로 구현을 택한다. 두 arm 모두 스킬 없이도 `git status` 확인이나 미커밋 변경 손실 경고 같은 대체 행동은 했다. 측정 원자료는 저장소 밖 `eval-tools/skill-trigger-test/`에 있다.
- 2026년 9월 18일 위 대응 뒤에도 Claude Code 세션이 CI·배포 구조 변경을 감사 없이 `main`에 push하고, 다른 세션이 provider 경계 변경을 거버넌스 스킬 없이 시작한 사례가 재발해 원인을 다시 진단했다. 확정한 원인은 접수 시점에 실패 영향을 판단하는 단계가 없다는 것이다. Claude Code 세션은 요청을 받으면 첫 행동이 곧 첫 구현 단계이고, 거버넌스는 실행 도중 훅 안내로만 끼어들어 무시되며(이 세션에서 안내 26회에 호출 0회), Codex 세션은 같은 지침·스킬에서 첫 추론이 위험 분류이고 그 자리에서 preflight와 감사를 고른 뒤 행동한다. v1.15.0 설명문 재작성이 판단 기준을 범주 목록으로 좁힌 것(5/17)과 v1.15.x 측정이 짧은 키워드 요청만 잰 것은 기여 요인이다. 판별 검사(같은 CI 변경 과제, 새 세션, `claude-fable-5-1`, 조건당 2~3회): 첫 편집 전 거버넌스 스킬을 스스로 호출한 비율은 현재 사본 0/2, Codex 원문 설명문 0/2, 프롬프트마다 orchestrator 지침 전문 주입 0/2였고, 프롬프트마다 접수 규칙 주입 2/2, MCP 서버 세션 `instructions`의 접수 규칙 2/2였다. 접수 규칙만으로는 orchestrator를 읽고도 필요한 단계를 정하지 않고 구현으로 갔으며(MCP 허용 2/2 동일), 과제를 `main` 병합·`origin` push까지로 올린 뒤 접수 규칙이 orchestrator를 거치게 하고 orchestrator에 선택 결정 절을 넣은 사본은 3/3에서 fresh 감사자 독립 감사를 거쳐 push했다(대조군 0/2, 턴 수 48~56→61~124, 그중 1회는 push 뒤 턴 한도로 종료). v1.16.1은 서버 `instructions`와 orchestrator 선택 결정 절을 Claude 배포물에만 적용한다. 원자료는 저장소 밖 `eval-tools/`(`skill-trigger-test/`, `release-1.16.1/`)와 세션 scratchpad에 있다.
...
- 2026년 9월 18일 MIT 공개 스킬 `ponytail`(원본 `DietrichGebert/ponytail` v4.10.0, 포크 `jaeseongs95/ponytail` 커밋 `83b2cbc`)을 v1.20.0에서 구현 단계 provider(`minimal-implementation`, phaseOrder 44)로 붙였다. 사용자 결정에 따라 원본의 항상 켜짐 훅, 보조 스킬 5개와 MCP 서버는 넣지 않았고 Claude·Codex 배포물 모두에 넣었다. Codex 공식 검사기가 허용하지 않는 frontmatter 키 `argument-hint`는 공용 사본에서 빼고 Claude 사본에서만 되살렸다. 이어서 사용자 결정으로 거버넌스 흐름 밖의 평소 코드 작업에도 적용했다. Claude는 세션 접수 규칙에 낮은 실패 영향의 코드 요청이면 ponytail을 먼저 호출하는 문장을, Codex는 설명문과 암시 호출 정책을 쓰며, 본문이 뒤 답변으로 번지지 않도록 공용 본문의 지속 범위와 경계 문단을 고쳤다.
- 2026년 9월 19일 다른 프로젝트의 Claude Code 대화형 세션에서 `plan_workflow`가 약 7초 뒤 항상 `BINDING_REQUIRED`로 거절되는 원인을 확인했다. 대화형 세션은 도구를 호출한 assistant 메시지를 호출이 끝난 뒤에 transcript에 쓰므로(실측: 메시지 24.6초, 거절 31.46초, 파일 기록 31.61초) 훅이 그 메시지를 찾지 못했고, v1.17.0 이후 검증은 모두 헤드리스여서 드러나지 않았다. 프로젝트 설정 문제가 아니었고 이 저장소의 대화형 세션에서도 재현됐다. v1.20.1에서 훅이 `SessionStart`·`PostModelSwitch`에서 기록한 세션 모델이나 이미 기록된 직전 메시지의 모델로 증명하게 했다. 추론 수준은 훅 입력에서만 가져오며, 대화형 세션에서 Opus 통과와 `/model` 전환 뒤 Sonnet 증명을 실측했다.
...
## 3.0 이후: 세션 작업 완료와 상태 기반 연락

기존 세션 presence는 실행 인스턴스의 온라인 여부를, 메시지 broker는 전달·ACK를 다룬다. 여기에 **작업의 종료 여부**를 별도 MCP 상태로 추가한다. 세션이 조용해졌거나 메시지가 전달됐다는 사실만으로 작업 완료를 추정하지 않는다.

### 구현 범위

- 작업 세션은 맡은 작업을 마칠 때 MCP에 task ID, host·session·instance ID, 최종 결과(`COMPLETED`, `BLOCKED`, `CANCELLED` 등), handoff·검증 근거 참조와 시각을 결속한 종료 신호를 반드시 제출한다. MCP가 수락한 기록을 확인한 뒤 결과를 통지한다. 이 기록은 종료 신고이며 실제 수용 기준 충족은 기존 증거 검증 절차에서 확정한다. 기록 실패나 비정상 종료는 `unknown`으로 남겨 재조회·복구하며 성공으로 보정하지 않는다.
- MCP는 작업 종료 기록을 세션 presence(`online`, `unreachable`, `ended`, `unknown`) 및 메시지 전달 ACK와 분리해 조회한다. 같은 작업의 중복 종료 호출은 멱등 처리하고, 오래된 instance나 더 이른 revision의 신호가 최신 상태를 덮지 못하게 한다.
- 다른 세션에 연락하기 직전에 수신 세션의 최신 presence, 실행 인스턴스, 작업 종료 기록, 입력 처리 상태와 wake 지원 여부를 조회한다. 온라인 presence만으로 작업 중인지 잠들었는지 판단하지 않는다. 입력을 처리 중인 세션에는 기존 broker로 메시지를 전달하고, 작업 종료 뒤 대기 중인 세션에는 메시지를 먼저 저장한 뒤 지원되는 wake 신호를 보낸다. 상태가 불명확하거나 wake를 지원하지 않으면 메시지를 보류·재확인하고 깨움이나 수신을 성공으로 표시하지 않는다.
- 다른 세션에 작업을 요청할 때는 요청 ID와 결과를 받을 세션을 지정하고 결과 callback을 요구한 뒤 발신 작업은 계속 진행한다. 수신 세션은 작업을 마치거나 차단되면 같은 요청 ID에 결과·근거 참조를 결속한 callback을 보낸다. 발신 세션은 완료 여부를 반복 조회하며 기다리지 않고 callback으로 재개한다. 기한까지 callback이 없을 때만 저장된 요청·종료 기록·전달 상태를 대조해 미확정 상태를 처리한다.
- 조회와 전송 사이 상태가 바뀔 수 있으므로 전송 시 인스턴스를 재검증한다. 중복 wake는 억제하고, 전달 완료는 수신 세션의 MCP ACK로만 확정한다. 종료 기록은 다른 세션의 새 작업을 자동 승인하거나 권한을 부여하지 않는다.

### 종료 기준

- 정상 완료·차단·취소·MCP 기록 실패·비정상 종료가 각각 정확한 작업 상태로 남는다.
- 작업 중 전달, 작업 종료 뒤 대기 세션 깨우기, wake 미지원·상태 불명, 조회 직후 인스턴스 교체와 중복 요청을 검사한다.
- callback 정상·차단·지연·중복·누락 사례에서 요청과 결과가 같은 ID로 추적되고, 발신 측의 반복 조회 없이 결과가 전달된다.
- 발신 측이 작업 종료와 메시지 저장·wake·수신 ACK를 서로 다른 단계로 조회할 수 있고, ACK 전에는 수신 완료로 표시하지 않는다.
