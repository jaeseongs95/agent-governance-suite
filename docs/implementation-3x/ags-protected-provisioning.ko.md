...
- **P3.2** 보호 주체를 설치하는 생산자(B14-q-a/B14-q, B14-r 등)는 installer가 작성하는 **설치 기록**을 둔다. 설치 기록의 필수 필드는 `contractId`, `revision`, 이 계약과 소비 Task 계약·fixture의 `sha256`, OS, installer·보호 주체·caller·worker의 실제 principal ID와 group·privilege 정책, 보호 파일·state·endpoint·실행물의 고정 절대 경로, OS 서비스 ID(서비스가 아닌 실행 형식이면 그 형식과 OS 등록 ID), 허용된 build digest와 launcher/interpreter closure digest다. 필드는 한 파일 또는 소비 Task가 정한 보호 파일 묶음에 모두 있어야 하며, 하나라도 없거나 조회하지 못하면 fail-closed다. 보호 주체와 preflight는 설치 기록을 읽어 실제 관측과 대조하며, 기록만으로 통과시키지 않는다. 설치 기록이 아닌 보호 파일(예: B14-m registry)의 schema와 B14-n 도구 무결성 기록(B14-m `toolIdentity`)은 해당 소비 계약이 정한다. self-reported JSON만으로 실효 ACL·token·hash 확인을 대체하지 않는다. 파일의 신뢰는 P2의 보호와 설치 측정에 의존한다.
...
- **P6.3** 보호 주체와 도구의 실행물·interpreter/launcher closure는 보호 subtree 또는 동등한 불변 설치 경로에서 측정한다. 사용자 쓰기 가능 DLL/module/search path, cwd, argv·env override, `.cmd` wrapper, 플러그인 설치 캐시·작업 트리를 보호 실행물로 인정하지 않는다. 선택 경로는 측정된 interpreter closure다: 보호된 `node.exe`/`node`, 진입 스크립트, 실행 인수, OS loader가 실제 읽는 dependency·module bytes를 후보별 manifest에 결속한다(Node 24+·pnpm 11·esbuild 기준, `node_modules` 없는 설치물). 소비 계약이 정한 OS helper(B14-m의 System32 `powershell.exe`·helper 스크립트 등)도 같은 측정·결속 대상이다. closure를 닫을 수 없으면 해당 leaf는 `NEEDS_SPLIT`으로 보고하고 지원 OS를 추정하지 않는다.
...
| B14-m storage 기대 ID (이미 동결) | V03-i에서 installer 신뢰 경계, 보호 subtree 경로, 절차 1·4만 직접 소비 중이며 이는 이 계약의 P2·P6.1·P6.4와 같은 내용이다. P7은 이 계약 P2.5(V03-i `고정 경로와 신뢰 경계`의 handle 기반 해석)와 B14-m 자체의 원자적 생성 규칙을 일반화한 것이며 B14-m의 세부 규칙이 우선한다(P1.4). 참조를 이 계약으로 옮길지는 B14-m 소유자의 후속 개정 대상이며 이 Task는 B14-m 문서를 고치지 않는다 |
