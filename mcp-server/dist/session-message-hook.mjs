...
var ms = y((fs2) => {
  "use strict";
  Object.defineProperty(fs2, "__esModule", { value: true });
...
var ModelRoutingStore = class {
  ...
  decision(id) {
    ...
    return row ? { request: JSON.parse(row.request_json), environment: JSON.parse(row.environment_json), decision: readDecisionPayload(row.payload, id), resolvedAt: row.resolved_at } : null;
  }
  ...
  recordApplication(input, observationToken, makeRecord, now) {
    ...
    return transaction(this.database, () => {
      ...
      assert(record3.schemaVersion === input.schemaVersion, "RECORD_VERSION_MISMATCH");
      ...
      return { record: record3, artifact: { kind: `model-application.v${input.schemaVersion[0]}`, uri: `ags-model-record:${record3.recordDigest.slice(7)}`, digest: record3.recordDigest } };
    });
  }
...
function catalogIndex(directory = defaultCatalogDirectory) {
  ...
  for (const p of index.providers) {
    ...
    assert(ORIGINS.includes(p.provider) && !seen.has(p.provider), "ORIGIN_EXCLUDED");
    seen.add(p.provider);
    assert(/^[a-f0-9]{64}$/u.test(p.sha256), "INVALID_CATALOG");
  }
  return index;
}
function loadCatalog({ directory = defaultCatalogDirectory, providers = null, role = null } = {}) {
  ...
  const models = records.flatMap((p) => {
    ...
    assert(shard.schemaVersion === "1.0.0" && shard.provider === p.provider && Array.isArray(shard.models), "INVALID_CATALOG");
...
if (process.argv[1] && import.meta.url === pathToFileURL2(process.argv[1]).href && import.meta.url.endsWith("/model-catalog.mjs")) {
  try {
...
var ModelRoutingServiceCore = class {
  ...
  application(recordDigest) {
    ...
    const record3 = this.store.application(recordDigest);
    assert(record3?.schemaVersion !== "3.0.0", "V3_RECORD_READER_UNAVAILABLE");
    return record3;
  }
  query(input) {
...
function assertSemanticRecordIntegrity(record3) {
  ...
  if (record3.observed) {
    ...
    same2(record3.observed.binding, record3.binding, "record.observed.binding");
    same2(record3.observed.target, record3.target, "record.observed.target");
    same2(record3.observed.decisionDigest, record3.decisionDigest, "record.observed.decisionDigest");
  }
}
function assertSemanticRecordBinding(record3, decision) {
  ...
  for (const field of fields) same2(record3[field], decision[field], `record.${field}`);
  same2(record3.dispatched, decision.selected, "record.dispatched");
}
...
var ModelRoutingWorkflowBridge = class {
  ...
  current(binding) {
    ...
    requireCondition(binding.candidateDigest === guarded.lease.targetDigest || guarded.proposal.frame.targetArtifacts.some((item) => item.digest === binding.candidateDigest), "Routing candidate is not in the frozen attempt frame.");
    return { receipt, guarded };
  }
  ...
  validatePeerHandoff(rawRequest, receiverActor = null) {
    ...
    requireCondition([...required].every((action) => authorization.allowedActions.includes(action) && !authorization.prohibitedActions.includes(action)), "Peer tools or filesystem exceed the local task authorization.");
  ...
  validateStageArtifacts(result) {
    ...
    for (const artifact of artifacts) {
      ...
      const { guarded } = this.current(record3.binding);
      requireCondition(
        !["high", "critical"].includes(guarded.proposal.taskEnvelope.riskLevel) || request.highRisk,
...
function validateCapabilityPublication(raw, validator, nowMs) {
  ...
  const identity = object2(value.identity);
...
// mcp-server/src/model-routing-host-hook.ts
var HOST_IDS = { codex: "openai-codex", "claude-code": "anthropic-claude-code" };
var SAFE_ID = /^[A-Za-z0-9][A-Za-z0-9._:-]{0,199}$/u;
...
function nativeRoutingSettings(input, host, models, readTranscript = readNativeTranscript) {
  ...
  if (host === "codex") {
    ...
    if (transcript) {
      for (const candidate of transcriptCandidates(transcript, session, agent)) {
        ...
        const exact2 = body ? findToolUseObservation(body, call, session, agent) : null;
        if (exact2) {
          model = exact2.model;
          effort = exact2.effort;
          break;
        }
      }
    }
  ...
  const nativeReasoning = known?.nativeKinds.includes("enum") && effort && ["low", "medium", "high", "xhigh", "max"].includes(effort) ? { kind: "enum", value: effort } : null;
  return {
...
function capabilitySnapshot(input, host, settings, deps) {
  ...
  const binding = observed ? [{
    ...
    modelOrigin: observed.modelOrigin,
    servingProvider: "unknown",
    accessPath: "unknown",
    nativeReasoning: settings.nativeReasoning ?? { kind: "not-exposed" },
    runtimeMode: "unknown",
    invocationSurface: "peer-session",
    observableFields: settings.nativeReasoning ? ["model", "reasoning"] : ["model"],
  ...
  const snapshot = {
    schemaVersion: "1.0.0",
    ...target,
    hostVersion: contract?.hostVersion ?? "unknown",
    adapterVersion: "p3-native-hook-v1",
    observedAt: now,
    expiresAt: new Date(Date.parse(now) + 6e4).toISOString(),
  ...
  validated.supportedBindings = validated.supportedBindings.map((item) => ({ ...item, observableFields: observed?.resolvedModel === item.resolvedModel && item.model === item.resolvedModel ? settings.nativeReasoning && canonicalJson(settings.nativeReasoning) === canonicalJson(item.nativeReasoning) ? ["model", "reasoning"] : ["model"] : [] }));
...
function handleNativeRoutingHook(input, host, deps) {
  ...
  const observation = {
    ...
    source: "host-event",
    reference: `native-tool-issuer:${convergenceDigest({ host, call: input.tool_use_id, event, application: convergenceDigest(application) })}`,
    observedAt: now,
    ...settings,
  ...
  if (event === "PostToolUse") {
    const result2 = object3(input.tool_response);
    const content = Array.isArray(result2.content) ? result2.content : [];
    const output = content.find((item) => object3(item).type === "text");
    const response = object3(JSON.parse(String(object3(output).text ?? "null")));
    check3(result2.isError !== true && response.ok === true, "The routing record call did not succeed.");
    previous = new ContractValidator().modelApplicationRecordV2(object3(response.data).record);
  ...
  const recorded = new ContractValidator().modelApplicationRecordV2(object3(result.data).record);
  return { hookSpecificOutput: { hookEventName: "PostToolUse", additionalContext: JSON.stringify({
    ...
    supersedesDiagnosticRecord: previous.recordDigest,
    artifact: object3(result.data).artifact,
    modelVerification: recorded.modelVerification,
    reasoningVerification: recorded.reasoningVerification,
    runtimeModeVerification: recorded.runtimeModeVerification,
    terminalOutcome: recorded.terminalOutcome,
    trustedGateSatisfied: false
  }) } };
}
...
// skills/coordinate-subagents/scripts/model-routing-peer.mjs
var MODEL_ROUTING_PEER_FEATURE = "model-routing.v2";
function encodePeerAssignment(request, decision, { delta = "", inputReferences = [] } = {}) {
  validateRequest(request);
  verifySeal(decision, "decisionDigest");
  assert(decision.status === "selected", "ASSIGNMENT_BLOCKED");
  ...
  assert(Buffer.byteLength(body, "utf8") <= 4096, "PEER_MESSAGE_TOO_LARGE");
  return body;
}
function decodePeerAssignment(body, negotiatedFeatures) {
  assert(Array.isArray(negotiatedFeatures) && negotiatedFeatures.includes(MODEL_ROUTING_PEER_FEATURE), "PEER_FEATURE_UNAVAILABLE");
  assert(typeof body === "string" && Buffer.byteLength(body, "utf8") <= 4096, "PEER_MESSAGE_TOO_LARGE");
  ...
  digestValue(v.decisionDigest, "decisionDigest");
...
var preflightPeerAssignment = revalidateDispatch;
async function acceptPeerAssignment(body, { features, target, loadAssignment, environment, authorizeAndConsume, revalidateAuthorization, store }) {
  const envelope = decodePeerAssignment(body, features);
  assert(canonical(envelope.target) === canonical(target), "PEER_TARGET_MISMATCH");
  ...
  let allowed = false;
  try {
    allowed = await authorizeAndConsume({ request, decision, target });
  } catch {
  ...
  if (allowed !== true) {
    store.transition(reservation.dispatchKey, 0, "not-started", "existing-workflow:authorization-denied");
    return { accepted: false, shouldExecute: false, reason: "EXISTING_WORKFLOW_AUTHORIZATION_REQUIRED" };
  }
  try {
    ...
    store.transition(reservation.dispatchKey, 0, "not-started", "existing-workflow:revalidation-failed");
    throw error;
  }
...
function peerIdentity(value) {
  const identity = peerObject(value);
  peerExact(identity, ["host", "sessionId", "instanceId"]);
  for (const [key, maximum] of [["host", 64], ["sessionId", 200], ["instanceId", 128]]) {
    peerCheck(typeof identity[key] === "string" && identity[key].length <= maximum && /^[A-Za-z0-9][A-Za-z0-9._:-]*$/u.test(identity[key]), "Invalid peer identity.");
  }
  return identity;
}
function peerInstant(value) {
...
function peerReceipt(packet) {
  ...
  peerCheck(typeof value.proposalId === "string" && /^ags-peer-[a-f0-9]{64}$/u.test(value.proposalId) && typeof value.decisionDigest === "string" && /^sha256:[a-f0-9]{64}$/u.test(value.decisionDigest) && typeof value.bindingDigest === "string" && /^sha256:[a-f0-9]{64}$/u.test(value.bindingDigest), "Invalid peer receipt binding.");
  peerCheck(["accepted", "rejected", "unknown"].includes(String(value.disposition)) && typeof value.reason === "string" && /^[A-Z_]{1,80}$/u.test(value.reason) && value.executionStarted === false && value.completed === false, "A handoff receipt cannot claim execution or completion.");
  return value;
}
var ModelPeerPacketSigner = class {
  key;
  ...
  verify(body, nowMs) {
    ...
    peerCheck(packet.schemaVersion === "1.0.0" && packet.feature === PEER_PACKET_FEATURE && ["proposal", "receipt"].includes(String(packet.kind)) && typeof packet.mac === "string" && /^[a-f0-9]{64}$/u.test(packet.mac), "Invalid peer packet version or signature.");
    peerIdentity(packet.sender);
    peerIdentity(packet.recipient);
...
function packetMatchesDecision(packet, decision) {
  const proposal = decodePeerAssignment(canonicalJson(packet.contents), [MODEL_ROUTING_PEER_FEATURE]);
  peerCheck(packet.kind === "proposal" && proposal.decisionDigest === decision.decisionDigest && canonicalJson(proposal.binding) === canonicalJson(decision.binding) && canonicalJson(proposal.target) === canonicalJson(decision.target), "Peer proposal differs from the locally registered decision.");
}

// mcp-server/src/model-peer-journal.ts
var ModelPeerJournal = class {
  constructor(database) {
    this.database = database;
  ...
  get(packetId, direction = "outbound") {
  ...
  prepare(direction, decision, body, write, expiresAt) {
    ...
    return this.transaction(() => {
      ...
      const count = this.database.prepare("SELECT COUNT(*) AS n FROM ags_model_peer_transfers_v1").get();
      peerCheck(count.n < 256, "Peer transfer journal is full; resolve retained transfers before new handoffs.");
      const b2 = decision.binding;
      const writeKey = direction === "outbound" && write ? convergenceDigest({ taskId: b2.taskId, runId: b2.runId, stageId: b2.stageId }) : null;
      this.database.prepare("INSERT INTO ags_model_peer_transfers_v1 VALUES (?,?,?,?,?,?,'prepared',NULL,?)").run(packetId, direction, decision.decisionDigest, convergenceDigest(b2), writeKey, body, expiresAt);
      return this.get(packetId, direction);
    });
  }
  claimInbound(packetId) {
  ...
  settle(packetId, disposition, reply, direction = "outbound") {
    return this.transaction(() => {
      const transfer = this.get(packetId, direction);
      peerCheck(transfer, "Unknown peer transfer.");
      if (transfer.reply !== null) {
        peerCheck(transfer.state === disposition && transfer.reply === reply, "Conflicting peer acceptance receipt.");
        return transfer;
      }
      this.database.prepare("UPDATE ags_model_peer_transfers_v1 SET state=?,reply=? WHERE packet_id=? AND direction=? AND reply IS NULL").run(disposition, reply, packetId, direction);
      return this.get(packetId, direction);
    });
  }
};
...
// mcp-server/src/model-routing-peer-session.ts
var ModelRoutingPeerSession = class {
  constructor(options) {
  ...
  clock;
  exchange() {
    const deadline = performance3.now() + (this.options.timeoutMs ?? 4e3);
    const request = (operation, payload, directory, timeout) => {
      const remaining = Math.min(timeout, deadline - performance3.now());
    ...
    const call = (operation, payload) => request(operation, payload, this.options.stateDirectory, deadline - performance3.now());
    return { call, request };
  }
  entry(id) {
    const entry = this.options.store.decision(id);
    peerCheck(entry, "Local assignment is unavailable.");
    ...
    const { decisionDigest, ...unsigned } = decision;
    peerCheck(
      decisionDigest === id && convergenceDigest(unsigned) === id && convergenceDigest(request) === decision.requestDigest && canonicalJson(request.binding) === canonicalJson(decision.binding) && decision.status === "selected" && decision.invocationSurface === "peer-session" && decision.target,
      "The stored peer decision is invalid or not a peer-session selection."
    );
    return { ...entry, decision };
  }
  async alive(identity, call) {
    const response = await call("presence", { target: { host: identity.host, sessionId: identity.sessionId } });
    const presence = response?.presence;
    peerCheck(presence && presence.host === identity.host && presence.sessionId === identity.sessionId && presence.instanceId === identity.instanceId && presence.state === "online" && presence.leaseUntil && peerInstant(presence.leaseUntil) > this.clock(), "Peer session is expired or its instance changed.");
    return presence;
  }
  async current(entry, io2) {
    const shared = await readSharedModelCapabilities(this.options.stateDirectory, { request: io2.request, clock: this.clock, timeoutMs: this.options.timeoutMs ?? 4e3 });
    ...
    const selected = shared.entries.filter((item) => item.snapshot.snapshotDigest === entry.decision.capabilitySnapshotDigest && canonicalJson({ host: item.snapshot.host, sessionId: item.snapshot.sessionId, instanceId: item.snapshot.instanceId, actorId: item.snapshot.actorId }) === canonicalJson(target));
    peerCheck(selected.length === 1, "Selected peer capability has changed or expired.");
    const remote = selected[0];
    const presence = await this.alive(remote.identity, io2.call);
    await this.alive(this.options.identity, io2.call);
  ...
  async send(decisionDigest, details = {}) {
    const entry = this.entry(decisionDigest), io2 = this.exchange();
    this.options.workflowBridge.validatePeerHandoff(entry.request);
    const current = await this.current(entry, io2);
    this.options.workflowBridge.validatePeerHandoff(entry.request);
    ...
    if (previous) {
      ...
      peerCheck(canonicalJson(existing.contents) === body && canonicalJson(existing.sender) === canonicalJson(this.options.identity) && canonicalJson(existing.recipient) === canonicalJson(current.transport), "A retry must use the identical approved handoff.");
      if (previous.state === "accepted" || previous.state === "rejected") return this.status(previous.packetId);
      packet = previous.body;
    } else {
      const now = this.clock();
      packet = this.options.signer.sign({
        kind: "proposal",
        sender: this.options.identity,
        recipient: current.transport,
        issuedAt: new Date(now).toISOString(),
        expiresAt: new Date(Math.min(now + 6e4, Date.parse(current.expiresAt))).toISOString(),
        contents: JSON.parse(body)
      });
    }
    const transfer = this.journal.prepare("outbound", entry.decision, packet, entry.request.requirements.filesystem === "write", JSON.parse(packet).expiresAt);
    try {
      await this.transmit(packet, io2);
      this.journal.sent(transfer.packetId);
    } catch {
      this.journal.unknown(transfer.packetId);
    }
    return this.status(transfer.packetId);
  }
  async status(packetId) {
    const transfer = this.journal.get(packetId);
    peerCheck(transfer?.direction === "outbound", "No outbound handoff has this ID.");
  ...
  async observeExecutionPreflight(packetId, expectedRevision = null) {
    const started = performance3.now(), io2 = this.exchange();
    ...
    const current = await this.current(before.entry, io2);
    ...
    const sender = await this.alive(before.packet.sender, io2.call);
    const recipient = await this.alive(before.packet.recipient, io2.call);
  ...
  async receive(message) {
    const io2 = this.exchange(), packet = this.options.signer.verifyMessage(message, this.options.identity, this.clock());
    await this.alive(packet.sender, io2.call);
    await this.alive(packet.recipient, io2.call);
    if (packet.kind === "receipt") {
      ...
      const old = JSON.parse(outbound.body);
      const proposal = this.options.signer.verify(outbound.body, peerInstant(old.issuedAt));
      peerCheck(canonicalJson(proposal.sender) === canonicalJson(packet.recipient) && canonicalJson(proposal.recipient) === canonicalJson(packet.sender) && proposal.contents.decisionDigest === receipt.decisionDigest && convergenceDigest(proposal.contents.binding) === receipt.bindingDigest, "Cross-task or cross-instance peer receipt.");
    ...
    peerCheck(envelope.target.actorId === this.options.actorId && envelope.target.sessionId === this.options.identity.sessionId && envelope.target.instanceId === this.options.identity.instanceId, "The handoff is not for the observed local actor.");
    const transfer = this.journal.prepare("inbound", envelope, message.body, false, packet.expiresAt);
    if (transfer.reply) {
      try {
        await this.transmit(transfer.reply, io2);
      } catch {
      }
    ...
    try {
      const entry = this.entry(envelope.decisionDigest);
      packetMatchesDecision(packet, entry.decision);
      const current = await this.current(entry, io2);
      ...
      const admitted = await acceptPeerAssignment(canonicalJson(packet.contents), {
        features: [MODEL_ROUTING_PEER_FEATURE],
        target: envelope.target,
        loadAssignment: async () => ({ request: entry.request, decision: entry.decision }),
        store: this.options.store,
        environment: { ...current.environment, refresh: async () => {
          this.options.signer.verify(message.body, this.clock());
          await this.alive(packet.sender, io2.call);
          return (await this.current(entry, io2)).environment;
        } },
        authorizeAndConsume: async () => {
          try {
            this.options.workflowBridge.validatePeerHandoff(entry.request, this.options.actorId);
            return true;
          } catch {
            return false;
          }
    ...
    const now = this.clock();
    const reply = this.options.signer.sign({
      kind: "receipt",
      sender: packet.recipient,
      recipient: packet.sender,
      issuedAt: new Date(now).toISOString(),
      expiresAt: new Date(now + 6e4).toISOString(),
      contents: {
        proposalId: transfer.packetId,
        decisionDigest: envelope.decisionDigest,
        bindingDigest: convergenceDigest(envelope.binding),
        disposition,
        reason,
        executionStarted: false,
        completed: false
...
// mcp-server/src/model-routing-peer-native.ts
async function openNativePeerSession(host, input) {
  ...
  peerCheck(input.instance_id === void 0 || input.instance_id === presence.instanceId, "Native session instance changed.");
  ...
  try {
    ...
    const session = new ModelRoutingPeerSession({
      ...
      identity,
      actorId: nativeRoutingActor(host, identity.sessionId, null),
      signer: new ModelPeerPacketSigner(token),
      stateDirectory: directory
    });
    const opened = database, openedWorkflow = workflow;
    return { session, close: () => {
      opened.close();
      openedWorkflow.close();
    } };
  } catch (error) {
    database?.close();
    workflow?.close();
