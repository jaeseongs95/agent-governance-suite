...
// mcp-server/src/host-integration/flowmarshal-profile-probe.ts
...
function activeRootIdentity(root, stored) {
  const digest2 = surfaceDigest(root);
  const observed = stored && stored.surfaceDigest === digest2 ? stored.identity : null;
  const wasInferred = (index) => observed !== null && stored.inferred[index] === true;
  const known = { root, legacy: stored === null, observedWorkspace: observed !== null, surfaceDigest: digest2 };
...
var InMemoryWorkflowStore = class {
  ...
  insertConvergenceRoot(root) {
    ...
    const actives = [...this.convergence.values()].filter((snapshot) => !["completed", "abandoned"].includes(snapshot.root.state)).map((snapshot) => {
...
import { createHash as createHash5, createPublicKey as createPublicKey2, randomBytes as randomBytes2, verify as verify2 } from "node:crypto";
...
function contractValidators() {
  ...
  addFormats(ajv2);
  for (const schema2 of Object.values(compiledContractSchemas)) {
    ajv2.addSchema(schema2);
  }
  const validators = Object.fromEntries(
    Object.entries(compiledContractSchemas).map(([name, schema2]) => [name, ajv2.getSchema(schema2.$id)])
  );
  validators.checkpointEvidenceRef = ajv2.getSchema(
    `${compiledContractSchemas.checkpointContextRequest.$id}#/$defs/evidenceRef`
  );
  validators.checkpointDeltaStateAck = ajv2.getSchema(
    `${compiledContractSchemas.checkpointDelta.$id}#/$defs/stateAck`
  );
  validators.checkpointDeltaTransportAck = ajv2.getSchema(
    `${compiledContractSchemas.checkpointDelta.$id}#/$defs/transportAck`
  );
  validators.continuitySnapshot = ajv2.getSchema(
...
var ContractValidator = class {
  ...
  modelApplicationRecordForDecisionV3(value, decisionValue) {
    const decision = this.modelRoutingDecisionV3(decisionValue);
    const record2 = this.modelApplicationRecordV3(value);
    assertSemanticRecordBinding(record2, decision);
    return record2;
  }
  ...
  referenceOnlyFixedTokens(rootDirectory, reference) {
    const schema2 = this.readBoundSchema(rootDirectory, reference, "reference-only output");
  ...
  assertSchemaFile(rootDirectory, reference, value, label) {
    ...
    for (const directory of [path3.join(root, "contracts"), this.skillSchemaRoot(root, reference.path)]) {
      for (const candidate of this.schemaFiles(directory)) {
        const schema2 = JSON.parse(readFileSync(candidate, "utf8"));
        const id = typeof schema2.$id === "string" ? schema2.$id : `file://${candidate.split(path3.sep).join("/")}`;
        if (!schemas.has(id)) schemas.set(id, schema2);
      }
    }
    for (const schema2 of schemas.values()) ajv2.addSchema(schema2);
    const targetId = typeof targetSchema.$id === "string" ? targetSchema.$id : void 0;
    const validate2 = (targetId ? ajv2.getSchema(targetId) : void 0) ?? ajv2.compile(targetSchema);
    if (!validate2(value)) {
...
import { createHash as createHash4, createHmac, createPublicKey, randomBytes, timingSafeEqual, verify } from "node:crypto";
...
function verifyFlowmarshalReceipt(input) {
  const envelope = record(input.envelope);
  if (!envelope || !Object.hasOwn(input.arguments, "_hostAttestation") || !exactKeys(envelope, ["body", "signature", "keyId"]) || canonicalJson(input.arguments._hostAttestation) !== canonicalJson(envelope)) {
  ...
  if (!exactKeys(body, [
    ...
    "expiresAt",
    "transport"
  ]) || body.version !== 2 || body.domain !== "fm-same-user-provider-terminal-to-governance-v1" || !profileBinding || !assertions || !exactKeys(profileBinding, ["profileId", "freezeIdentity"]) || profileBinding.profileId !== input.profile.profileId || profileBinding.freezeIdentity !== input.profile.freezeIdentity || !exactKeys(assertions, ["modelClass", "actorId"]) || !nonempty(assertions.modelClass) || !nonempty(assertions.actorId) || !producer || !registeredProducer || !binding || !registeredBinding || !terminal || !core || !invocation || !transport || !exactKeys(transport, ["serverEpoch", "registrationDigest"]) || transport.serverEpoch !== input.serverEpoch || transport.registrationDigest !== input.registrationDigest || registration.serverEpoch !== input.serverEpoch || !exactKeys(binding, ["invocationId", "turnId", "taskId", "runId", "attemptId", "hostId", "sessionId", "instanceId"]) || binding.invocationId !== input.callId || producer.keyId !== keyId || producer.hostId !== "flowmarshal" || binding.hostId !== "flowmarshal" || !nonempty(body.nonce) || !nonempty(terminal.model) || !isReasoningEffort(terminal.effort) || !nonempty(terminal.eventId) || !nonempty(terminal.callId) || typeof terminal.status !== "string" || !["succeeded", "completed"].includes(terminal.status) || typeof terminal.provenance !== "string" || !["provider_raw_response", "claude_session_transcript"].includes(terminal.provenance) || typeof terminal.digest !== "string" || !/^sha256:[0-9a-f]{64}$/u.test(terminal.digest) || invocation.tool !== input.tool) {
  ...
  if (!Number.isFinite(input.now) || !Number.isFinite(issued) || !Number.isFinite(expires) || input.now < issued - CLOCK_SKEW_MS || input.now >= expires || expires - issued !== CHALLENGE_TTL_MS || terminalTime === null || terminalTime >= BigInt(issued) * 1000n + 1000n || BigInt(issued) * 1000n - terminalTime > BigInt(OBSERVATION_MAX_AGE_MS) * 1000n || invocation.observedAt !== body.issuedAt) {
    throw invalid("A2 receipt causal time or expiry is invalid");
  }
  const receiptDigest = `sha256:${createHash4("sha256").update(bytes).digest("hex")}`;
  return {
    profileId: "flowmarshal-same-user-v1",
    freezeIdentity: input.profile.freezeIdentity,
...
var PROFILE_ID = "flowmarshal-same-user-v1";
function reject(reason) {
  throw new Error(`FlowMarshal A2 dispatch unavailable: ${reason}`);
}
var object2 = (value) => value !== null && typeof value === "object" && !Array.isArray(value) ? value : null;
var exact = (value, keys) => !!value && Object.keys(value).length === keys.length && keys.every((key) => Object.hasOwn(value, key));
var required = (value) => typeof value === "string" && value.trim().length > 0;
var optional = (value) => value === null || required(value);
var timestamp2 = (value) => {
  if (typeof value !== "string") return NaN;
...
var safeJson = (value) => {
  ...
  const record2 = object2(value);
  return !!record2 && Object.values(record2).every(safeJson);
};
var FlowmarshalCurrentInvocation = class {
  constructor(profile, store, clock = Date.now) {
    ...
    this.store = store;
    this.clock = clock;
    if (profile.profileId !== PROFILE_ID || profile.assuranceTier !== "same-user" || !/^sha256:[0-9a-f]{64}$/u.test(profile.freezeIdentity) || profile.resources.state.namespace !== PROFILE_ID) reject("A2 profile is unavailable");
    ...
    let created = false;
    try {
      const fd = openSync(state, constants.O_CREAT | constants.O_EXCL | constants.O_WRONLY, 384);
      closeSync(fd);
      created = true;
    } catch (error) {
      if (error.code !== "EEXIST") reject("A2 state file cannot be created");
    }
  ...
  store;
  clock;
  serverEpoch = randomBytes2(32).toString("base64url");
  profileBinding;
  database;
  ...
  observe(binding) {
    ...
    const signedBinding = object2(current.reservation.body.binding);
    ...
    } else if (current.tool !== "record_stage_result" || current.arguments.stageId !== binding.stageId || current.arguments.expectedRevision !== binding.revision) {
  ...
  verifySignedEnvelope(value) {
    ...
    const key = createPublicKey2({ key: Buffer.from(pin.publicKeySpki, "base64"), format: "der", type: "spki" });
    if (signature.length !== 64 || !verify2(null, bytes, key, signature)) reject("registration signature mismatch");
    let decoded;
    try {
      decoded = JSON.parse(bytes.toString("utf8"));
    } catch {
      reject("registration body is malformed");
    }
    const body = object2(decoded);
  ...
  reserve(signedRegistration) {
    ...
    const profileBinding = object2(body.profileBinding), assertions = object2(body.assertions);
    const producer = object2(body.producer), binding = object2(body.binding);
    const terminal = object2(body.terminal), core = object2(body.core), invocation = object2(body.invocation);
    const now = this.clock(), issued = timestamp2(body.issuedAt), expires = timestamp2(body.expiresAt);
    const observed = timestamp2(terminal?.observedAt);
    if (!exact(body, ["version", "domain", "profileBinding", "assertions", "serverEpoch", "nonce", "issuedAt", "expiresAt", "producer", "binding", "terminal", "core", "invocation"]) || body.version !== 1 || body.domain !== DISPATCH_DOMAIN || !exact(profileBinding, ["profileId", "freezeIdentity"]) || profileBinding.profileId !== PROFILE_ID || profileBinding.freezeIdentity !== this.profile.freezeIdentity || !exact(assertions, ["modelClass", "actorId"]) || !required(assertions.modelClass) || !required(assertions.actorId) || body.serverEpoch !== this.serverEpoch || !required(body.nonce) || !Number.isFinite(now) || !Number.isFinite(issued) || !Number.isFinite(expires) || issued > now + 5e3 || now >= expires || expires - issued !== 6e4 || !exact(producer, ["installationId", "keyId", "hostId", "instanceId"]) || producer.keyId !== keyId || producer.hostId !== "flowmarshal" || !required(producer.installationId) || !required(producer.instanceId) || !exact(binding, ["turnId", "taskId", "runId", "attemptId", "hostId", "sessionId", "instanceId"]) || !required(binding.turnId) || !required(binding.taskId) || !optional(binding.runId) || !optional(binding.attemptId) || binding.hostId !== "flowmarshal" || !required(binding.sessionId) || binding.instanceId !== producer.instanceId || !exact(terminal, ["eventId", "callId", "threadId", "turnId", "status", "observedAt", "model", "effort", "provenance", "digest"]) || terminal.turnId !== binding.turnId || !required(terminal.eventId) || !required(terminal.callId) || !required(terminal.threadId) || !required(terminal.model) || !REASONING_EFFORT.includes(String(terminal.effort)) || !["succeeded", "completed"].includes(String(terminal.status)) || !["provider_raw_response", "claude_session_transcript"].includes(String(terminal.provenance)) || !/^sha256:[0-9a-f]{64}$/u.test(String(terminal.digest)) || !Number.isFinite(observed) || observed > issued || !exact(core, ["goalRevision", "taskRevision", "attemptOrdinal", "gateOperationKey", "stage"]) || !Number.isSafeInteger(core.goalRevision) || Number(core.goalRevision) < 1 || !Number.isSafeInteger(core.taskRevision) || Number(core.taskRevision) < 1 || !(core.attemptOrdinal === null || Number.isSafeInteger(core.attemptOrdinal) && Number(core.attemptOrdinal) > 0) || !required(core.gateOperationKey) || !required(core.stage) || !["bootstrap", "baseline", "implementation", "scope", "acceptance"].includes(core.stage) || !exact(invocation, ["tool", "inputDigest", "observedAt"]) || !["plan_workflow", "record_stage_result"].includes(String(invocation.tool)) || !/^sha256:[0-9a-f]{64}$/u.test(String(invocation.inputDigest)) || invocation.observedAt !== body.issuedAt || core.stage === "bootstrap" && (invocation.tool !== "plan_workflow" || binding.runId !== null || binding.attemptId !== null) || core.stage !== "bootstrap" && (invocation.tool !== "record_stage_result" || !required(binding.runId)) || ["bootstrap", "baseline"].includes(String(core.stage)) && binding.attemptId !== null || ["implementation", "scope", "acceptance"].includes(String(core.stage)) && !required(binding.attemptId)) {
      reject("registration binding is invalid");
    }
    const callId = `fmr-${randomBytes2(24).toString("base64url")}`;
    ...
    try {
      ...
      reject("registration nonce already reserved or A2 state unavailable");
    }
  ...
  reservation(callId) {
    ...
    if (JSON.stringify(verified.body) !== row.body_json || `sha256:${createHash5("sha256").update(verified.bytes).digest("hex")}` !== row.registration_digest || object2(verified.body.profileBinding)?.profileId !== PROFILE_ID || object2(verified.body.profileBinding)?.freezeIdentity !== this.profile.freezeIdentity || verified.body.serverEpoch !== this.serverEpoch || row.nonce_key !== `${PROFILE_ID}:${verified.keyId}:${verified.body.nonce}` || row.expires_at !== timestamp2(verified.body.expiresAt)) reject("stored registration evidence mismatch");
    return {
      body: verified.body,
      envelope,
  ...
  readCurrentInvocation() {
    const current = this.current.getStore();
    if (!current || !this.active.has(current.callId) || this.clock() >= current.reservation.expiresAt || this.reservation(current.callId)?.digest !== current.reservation.digest || this.reservation(current.callId)?.used) reject("current reserved request is unavailable");
    const body = current.reservation.body, binding = object2(body.binding);
    const core = object2(body.core), invocation = object2(body.invocation);
    ...
    if (current.tool === "plan_workflow") {
      ...
      if (task.taskId !== binding.taskId || binding.runId !== null || binding.attemptId !== null || core.stage !== "bootstrap") reject("bootstrap task binding mismatch");
      ...
      if (!run || run.state !== "running" || run.revision !== result.expectedRevision || run.plan.taskId !== binding.taskId || result.runId !== binding.runId || !stage || stage.state !== "ready") reject("stored workflow stage binding mismatch");
...
// mcp-server/src/host-integration/flowmarshal-profile.ts
var import__2 = __toESM(require__(), 1);
import { execFileSync } from "node:child_process";
import { createPublicKey as createPublicKey3 } from "node:crypto";
import { closeSync as closeSync2, constants as constants2, fstatSync, lstatSync as lstatSync2, openSync as openSync2, readFileSync as readFileSync2 } from "node:fs";
...
var schema = contractSchemas.hostIntegration;
var ajv = new import__2.Ajv2020().addSchema(schema);
var validateSelection = ajv.getSchema(`${String(schema.$id)}#/$defs/serverProfileSelection`);
var hasKeys = (value, keys) => value !== null && typeof value === "object" && !Array.isArray(value) && Object.keys(value).length === keys.length && keys.every((key) => Object.hasOwn(value, key));
var fail = (message) => {
  throw new Error(`FlowMarshal same-user profile unavailable: ${message}`);
};
function sameFile(a, b) {
  return a.dev === b.dev && a.ino === b.ino && a.mode === b.mode && a.uid === b.uid && a.gid === b.gid && a.size === b.size && a.mtimeMs === b.mtimeMs && a.ctimeMs === b.ctimeMs;
}
function isSameUserWindowsAcl(value) {
  if (!hasKeys(value, ["self", "owner", "rules"]) || typeof value.self !== "string" || value.owner !== value.self || !Array.isArray(value.rules)) return false;
  return value.rules.every((entry) => {
    if (!hasKeys(entry, ["sid", "rights", "type"]) || typeof entry.sid !== "string" || !Number.isInteger(entry.rights) || !["Allow", "Deny"].includes(String(entry.type))) return false;
    return entry.type === "Deny" || entry.sid === value.self || entry.sid === "S-1-5-18" || entry.sid === "S-1-5-32-544";
  });
}
function sameUserWindowsAcl(file) {
  const script = `$ErrorActionPreference='Stop'; $p=[Console]::In.ReadToEnd(); $a=if ([IO.Directory]::Exists($p)) { [IO.Directory]::GetAccessControl($p) } else { [IO.File]::GetAccessControl($p) }; $self=[Security.Principal.WindowsIdentity]::GetCurrent().User.Value; $rules=@($a.GetAccessRules($true,$true,[Security.Principal.SecurityIdentifier]) | ForEach-Object { @{ sid=$_.IdentityReference.Value; rights=[int]$_.FileSystemRights; type=$_.AccessControlType.ToString() } }); @{ self=$self; owner=$a.GetOwner([Security.Principal.SecurityIdentifier]).Value; rules=$rules } | ConvertTo-Json -Compress -Depth 4`;
  try {
...
function inspectFile(file, required2) {
  let status;
  try {
    status = lstatSync2(file);
  } catch (error) {
    if (!required2 && error.code === "ENOENT") return null;
    return fail(`missing or unreadable file: ${path4.basename(file)}`);
  }
  if (!status.isFile() || status.isSymbolicLink() || status.nlink !== 1) fail(`unsafe file: ${path4.basename(file)}`);
  if (process.platform !== "win32" && (status.uid !== process.getuid?.() || (status.mode & 63) !== 0)) {
    fail(`permissions are too broad: ${path4.basename(file)}`);
  }
  if (process.platform === "win32" && !sameUserWindowsAcl(file)) fail(`permissions are too broad: ${path4.basename(file)}`);
  return status;
}
function readTrustedJson(file) {
  const before = inspectFile(file, true);
  if (before.size > 1024 * 1024) fail(`file is too large: ${path4.basename(file)}`);
  let fd;
  try {
    fd = openSync2(file, constants2.O_RDONLY | (process.platform === "win32" ? 0 : constants2.O_NOFOLLOW));
  } catch {
    return fail(`file changed before open: ${path4.basename(file)}`);
  }
  try {
    if (!sameFile(before, fstatSync(fd))) fail(`file changed before read: ${path4.basename(file)}`);
    let parsed;
    try {
      parsed = JSON.parse(readFileSync2(fd, "utf8"));
    } catch {
      return fail(`invalid JSON: ${path4.basename(file)}`);
    }
    if (!sameFile(before, fstatSync(fd)) || !sameFile(before, inspectFile(file, true))) {
      fail(`file changed during read: ${path4.basename(file)}`);
    }
    return parsed;
  } finally {
    closeSync2(fd);
...
function parsePins(value) {
  if (!hasKeys(value, ["namespace", "pins"]) || value.namespace !== PROFILE_ID2 || !Array.isArray(value.pins) || value.pins.length === 0) fail("pin set is malformed");
  const pinSet = value;
  const seen = /* @__PURE__ */ new Set();
  let active = false;
  for (const item of pinSet.pins) {
    if (!hasKeys(item, ["keyId", "publicKeySpki", "status"]) || typeof item.keyId !== "string" || !item.keyId.trim() || seen.has(item.keyId) || typeof item.publicKeySpki !== "string" || !item.publicKeySpki || !["active", "revoked"].includes(String(item.status))) fail("pin set is malformed");
    ...
    try {
      if (createPublicKey3({ key: bytes, format: "der", type: "spki" }).asymmetricKeyType !== "ed25519") {
...
function loadFlowmarshalProfileFile(profilePath) {
  try {
    lstatSync2(profilePath);
  } catch (error) {
    if (error.code === "ENOENT") return null;
    return fail("profile file is unreadable");
  }
  const directory = path4.dirname(profilePath);
  const dir = lstatSync2(directory);
  if (!dir.isDirectory() || dir.isSymbolicLink()) fail("profile directory is unsafe");
  if (process.platform !== "win32" && (dir.uid !== process.getuid?.() || (dir.mode & 63) !== 0)) {
  ...
  const resources = {
    key: { namespace: PROFILE_ID2, location: path4.join(directory, "producer-key.json") },
    pin: { namespace: PROFILE_ID2, location: path4.join(directory, "pins.json") },
    state: { namespace: PROFILE_ID2, location: path4.join(directory, "state.sqlite3") }
  };
  const document = readTrustedJson(profilePath);
  if (!hasKeys(document, ["version", "selection", "resources"]) || document.version !== 1 || !validateSelection(document.selection) || !hasKeys(document.selection, ["source", "profile", "pinSetDigest", "resourceBindingDigest", "freezeIdentity"]) || !hasKeys(document.resources, ["key", "pin", "state"]) || convergenceDigest(document.resources) !== convergenceDigest(resources)) fail("selection or resources are malformed");
  const selected = document.selection;
  if (!hasKeys(selected.profile, ["profileId", "assuranceTier", "hostId", "receiptDomain", "dispatchDomain", "modelClassSource", "actorSource", "keyNamespace", "pinNamespace", "stateNamespace"]) || selected.profile.profileId !== PROFILE_ID2 || selected.profile.assuranceTier !== "same-user") {
    fail("protected VM profile cannot be selected");
  }
  const { freezeIdentity, ...selectionBody } = selected;
  if (freezeIdentity !== convergenceDigest(selectionBody) || selected.resourceBindingDigest !== convergenceDigest(resources)) fail("profile freeze identity is invalid");
  inspectFile(resources.key.location, true);
  inspectFile(resources.state.location, false);
  ...
  return Object.freeze({
    ...
    freezeIdentity: String(freezeIdentity),
    pins: Object.freeze(pins.pins.map((pin) => Object.freeze({ ...pin }))),
    resources: Object.freeze({
      key: Object.freeze(resources.key),
      pin: Object.freeze(resources.pin),
      state: Object.freeze(resources.state)
    })
  });
}

// mcp-server/src/host-integration/manifest.ts
var import__3 = __toESM(require__(), 1);
import { createHash as createHash6 } from "node:crypto";
import { readFileSync as readFileSync3, readdirSync as readdirSync2, realpathSync, statSync as statSync2 } from "node:fs";
import path5 from "node:path";
var HOST_INTEGRATION_FORMAT = "agent-governance-suite.host-integration.v1";
var validate = new import__3.Ajv2020({ allErrors: true }).compile(contractSchemas.hostIntegration);
function resolvePackageFile(root, relativePath) {
  if (relativePath.includes("\\") || relativePath.includes(":") || relativePath.startsWith("/") || relativePath.split("/").some((part) => part === "" || part === "." || part === "..")) {
    throw new Error(`Invalid package-relative path: ${relativePath}`);
  }
  const target = path5.resolve(root, relativePath);
  let actual;
  try {
    actual = realpathSync(target);
    if (!statSync2(actual).isFile()) throw new Error("not a regular file");
  } catch {
    throw new Error(`Missing package file: ${relativePath}`);
  }
  const relative = path5.relative(root, actual);
...
function parseHostIntegrationManifest(input, rootDirectory) {
  ...
  if (manifest.artifacts) {
    ...
    for (const artifact of manifest.artifacts) {
      ...
      const actual = `sha256:${createHash6("sha256").update(readFileSync3(file)).digest("hex")}`;
...
function buildHostIntegrationDescriptor(rootDirectory, candidates) {
  const root = realpathSync(rootDirectory);
  const entryPoints = [];
  ...
  const artifacts = [...new Set(entryPoints.flatMap((entry) => entry.executionClosure))].sort().map((file) => ({
    path: file,
    sha256: `sha256:${createHash6("sha256").update(readFileSync3(resolvePackageFile(root, file))).digest("hex")}`
  }));
  const manifest = { format: HOST_INTEGRATION_FORMAT, artifacts, entryPoints };
  parseHostIntegrationManifest(manifest, root);
  return { manifest, disabledEntryPoints };
}
function filesIn(root, directory) {
  return readdirSync2(path5.join(root, directory), { withFileTypes: true }).flatMap((entry) => {
    const relative = `${directory}/${entry.name}`;
    return entry.isDirectory() ? filesIn(root, relative) : entry.isFile() ? [relative] : [];
  });
}
function assertSchemaReferences(root, schemaPaths, providerPaths) {
  const byId = /* @__PURE__ */ new Map();
  const byPath = /* @__PURE__ */ new Map();
  for (const file of schemaPaths) {
    const schema2 = JSON.parse(readFileSync3(resolvePackageFile(root, file), "utf8"));
    byPath.set(file, schema2);
    byId.set(`file:///${file}`, schema2);
    if (schema2.$id && !byId.has(schema2.$id)) byId.set(schema2.$id, schema2);
  }
  const visited = /* @__PURE__ */ new Set();
  const visit = (value, base) => {
    if (!value || typeof value !== "object") return;
    if (Array.isArray(value)) {
    ...
    const record2 = value;
    const currentBase = typeof record2.$id === "string" ? new URL(record2.$id, base).href : base;
    if (typeof record2.$ref === "string") {
      const target = new URL(record2.$ref, currentBase);
      target.hash = "";
    ...
    for (const item of Object.values(record2)) visit(item, currentBase);
  };
  for (const file of providerPaths) {
    const schema2 = byPath.get(file);
    if (!schema2) throw new Error(`Missing provider schema: ${file}`);
    visit(schema2, schema2.$id ?? `file:///${file}`);
  }
}
var IMPORT_SPECIFIER = /\bfrom\s*["']([^"']+)["']|\bimport\s*\(?\s*["']([^"']+)["']/g;
function importClosure(root, file, seen = /* @__PURE__ */ new Set()) {
  if (seen.has(file)) return seen;
  seen.add(file);
  const source = readFileSync3(resolvePackageFile(root, file), "utf8");
  ...
  for (const match of source.matchAll(IMPORT_SPECIFIER)) {
    ...
    importClosure(root, path5.posix.normalize(path5.posix.join(path5.posix.dirname(file), specifier)), seen);
  }
  return seen;
}
var SCRIPT_ENTRY_POINTS = [
...
function buildCurrentHostIntegrationManifest(rootDirectory) {
  const root = realpathSync(rootDirectory);
  const mcp = JSON.parse(readFileSync3(path5.join(root, ".mcp.json"), "utf8"));
  const server = mcp.mcpServers?.["agent-governance-suite"];
  if (server?.command !== "node" || server.args?.length !== 1 || server.args[0] !== "mcp-server/dist/server.mjs" || server.cwd !== ".") {
    throw new Error("Packaged MCP configuration does not match the direct server entry point");
  }
  const plugin = JSON.parse(readFileSync3(path5.join(root, ".codex-plugin/plugin.json"), "utf8"));
  const registry = JSON.parse(readFileSync3(path5.join(root, "skills/registry.json"), "utf8"));
  const providerSchemas = registry.skills.flatMap((skill) => skill.providers.flatMap((provider) => [provider.outputSchema, provider.resultSchema, provider.gate?.validator].filter((file) => typeof file === "string" && file.endsWith(".schema.json"))));
  const skillSchemas = filesIn(root, "skills").filter((file) => file.endsWith(".schema.json"));
  const contractSchemas2 = filesIn(root, "contracts").filter((file) => file.endsWith(".json"));
  assertSchemaReferences(root, [...contractSchemas2, ...skillSchemas], providerSchemas);
  const executionClosure = [.../* @__PURE__ */ new Set([
    ".mcp.json",
  ...
  const scripts = SCRIPT_ENTRY_POINTS.map(([id, entry]) => {
    const skillRoot = entry.split("/").slice(0, 2).join("/");
    const schemas = filesIn(root, skillRoot).filter((file) => file.endsWith(".schema.json"));
    return { id, path: entry, executionClosure: [.../* @__PURE__ */ new Set([...importClosure(root, entry), ...schemas])].sort(), enabled: true };
  });
  const descriptor = buildHostIntegrationDescriptor(root, [{
    id: "mcp-server",
    path: "mcp-server/dist/server.mjs",
    executionClosure,
    enabled: true
  }, ...scripts]);
...
// mcp-server/src/host-integration/flowmarshal-profile-probe.ts
var PROFILE_ID3 = "flowmarshal-same-user-v1";
var PROBE_PATH = "mcp-server/dist/flowmarshal-profile-probe.mjs";
function probeFlowmarshalProfileState(packageRoot) {
  const root = realpathSync2(packageRoot);
  const manifestBytes = readFileSync4(path6.join(root, "host-integration.json"));
  ...
  if (entry?.path !== "mcp-server/dist/server.mjs" || !entry.executionClosure.includes(PROBE_PATH) || !manifest.artifacts?.some((artifact) => artifact.path === PROBE_PATH)) {
    throw new Error("FlowMarshal A2 probe or MCP entry point is not in the package manifest closure");
  }
  const profile = loadFlowmarshalProfileFile(path6.join(root, PROFILE_ID3, "server-profile.json"));
  if (!profile || profile.profileId !== PROFILE_ID3) throw new Error("FlowMarshal A2 profile is unavailable");
  ...
  try {
    ...
    try {
      ...
      unsignedRegistrationRejected = error instanceof Error && error.message.includes("signed registration is malformed");
    }
    let unreservedExecutionRejected = false;
    try {
      invocation.readCurrentInvocation();
    } catch (error) {
      unreservedExecutionRejected = error instanceof Error && error.message.includes("current reserved request is unavailable");
    }
    ...
    if (invocation.hasCurrentRequest() || !unsignedRegistrationRejected || !unboundObservationRejected || !unreservedExecutionRejected) {
      throw new Error("FlowMarshal A2 invocation surfaces are not isolated");
    }
    return {
      status: "PASS",
      evidenceClass: "local_derived",
      profileId: PROFILE_ID3,
      freezeIdentity: profile.freezeIdentity,
      manifestSha256: `sha256:${createHash7("sha256").update(manifestBytes).digest("hex")}`,
      surfaceChecks: {
...
if (process.argv[1] && path6.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
