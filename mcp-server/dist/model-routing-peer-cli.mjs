...
function rankedBaselineEntriesV2(candidates, request, { catalog, policy }) {
  ...
  return structuredClone(candidates).map((candidate) => ({ candidate, order: rank(candidate) })).sort((a, b) => {
...
var ModelPeerJournal = class {
  ...
  prepare(direction, decision, body, write, expiresAt) {
    ...
    return this.transaction(() => {
      ...
      this.database.prepare("INSERT INTO ags_model_peer_transfers_v1 VALUES (?,?,?,?,?,?,'prepared',NULL,?)").run(packetId, direction, decision.decisionDigest, convergenceDigest(b), writeKey, body, expiresAt);
...
var ModelRoutingPeerSession = class {
  ...
  async current(entry, io) {
    const shared = await readSharedModelCapabilities(this.options.stateDirectory, { request: io.request, clock: this.clock, timeoutMs: this.options.timeoutMs ?? 4e3 });
  ...
  async receive(message) {
    ...
    if (transfer.reply) {
      try {
        await this.transmit(transfer.reply, io);
      } catch {
      }
