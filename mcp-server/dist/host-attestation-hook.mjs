...
function activeRootIdentity(root, stored) {
  ...
  if (observed && observed.surfaces.every((surface) => surface.git !== null)) {
    return { root, identity: observed, resolved: true, legacy, fresh: false, surfaceDigest: digest2 };
  }
  let derived;
  try {
    ...
    if (observed) return { root, identity: observed, resolved: true, legacy, fresh: false, surfaceDigest: digest2 };
    ...
    return { root, identity: identity2, resolved: false, legacy, fresh: false, surfaceDigest: digest2 };
  }
  const identity = {
    ...derived,
  ...
  return { root, identity, resolved, legacy, fresh, surfaceDigest: digest2 };
}
...
function transcriptCandidates(transcriptPath, sessionId, agentId) {
  ...
  if (agentId) {
    candidates.push(path5.join(path5.dirname(transcriptPath), sessionId, "subagents", `agent-${agentId}.jsonl`));
  }
...
function sessionModelFile(directory, sessionId) {
  return path5.join(directory, `${digest(sessionId)}.json`);
}
