...
export class ModelRoutingWorkflowBridge {
  ...
  validatePeerExecutionPreflight(rawRequest: unknown, receiverActor: string): void {
    this.validatePeerHandoff(rawRequest, receiverActor);
    const request = this.validator.modelSelectionRequestV2(rawRequest);
    const { guarded } = this.current(request.binding);
    requireCondition(guarded.proposal.taskEnvelope.authorization.approvalRequired.length === 0,
